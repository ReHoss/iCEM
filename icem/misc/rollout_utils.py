...
from icem.environments.abstract_environments import (
...
class RolloutManager:
    ...
    def __init__(self, env, roll_params, string_to_env_map):
        self.env = env
        self.task_horizon = roll_params.task_horizon
        self.record = roll_params.record
        self.only_final_reward = (
            False
            if "only_final_reward" not in roll_params
            else roll_params.only_final_reward
        )
        ...
        self.num_parallel = (
            roll_params.num_parallel if "num_parallel" in roll_params else 1
        )
        self.parallel_training = (
            roll_params.parallel_training
            if "parallel_training" in roll_params
            else False
        )
    ...
    def setup_video(self, name_suffix=""):
        ...
        file_path = os.path.join(
            self.dir_name, f"rollout_{name_suffix}_{0}_{self.calls_counter:02d}.mp4"
        )
        i = 0
        while os.path.isfile(file_path):
            i += 1
            file_path = os.path.join(
                self.dir_name, f"rollout_{name_suffix}_{i}_{self.calls_counter:02d}.mp4"
            )
    ...
    def sample(
        ...
        if hasattr(self.env, "dmcenv"):
            ...
            temp_start_state = (
                [None] * no_rollouts if start_state is None else start_state
            )
            return [
                ...
                for i in (
                    tqdm_context(range(no_rollouts), desc=desc)
                    if use_tqdm
            ...
            if self.do_run_in_parallel(policy, mode):
                ...
                return self.par_sample(
                    policy, render, mode, start_ob, start_state, no_rollouts
                )
            else:
                temp_start_ob = [None] * no_rollouts if start_ob is None else start_ob
                temp_start_state = (
    ...
    def par_sample(
        ...
        if "MujocoException" in rollout_list:
            raise Exception("Got MujocoException")
        ...
        for t in (
            ...
            if render:
                if video is not None:
                    ...
                    # CHANGES @ReHoss: Start - render mode remove argument
                    frame = env.render()
                    # CHANGES @ReHoss: End
            ...
            if isinstance(env, GoalSpaceEnvironmentInterface):
                transition.append(
                    env.is_success(ob[None, :], ac[None, :], next_ob[None, :])[0]
                )
    ...
    def sample_deepmindsuite(
        self, policy, logger, render: bool, mode, start_ob, start_state
    ):
        ...
        for t in tqdm(range(self.task_horizon), desc="time_steps"):
            ...
            if isinstance(self.env, GoalSpaceEnvironmentInterface):
                transition.append(
                    self.env.is_success(ob[None, :], ac[None, :], next_ob[None, :])[0]
                )
    ...
    @staticmethod
    def worker(seed, worker_id, remote, parent_remote, env_wrapper, string_to_env_map):
        parent_remote.close()
        orig_env = env_wrapper.x
        env = string_to_env_map(orig_env.name, orig_env.init_kwargs)
        ...
        try:
            while True:
                ...
                if cmd == "_sample":
                    ...
                    for start_ob, start_state in zip(
                        data["start_obs"], data["start_obs"]
                    ):
