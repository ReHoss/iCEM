...
from icem.misc.helpers import (
    ...
    compute_and_log_reward_info,
    update_reward_dict,
    update_from_cmd_line,
    save_settings_to_json,
)
from icem.misc.initialization import CheckpointManager
from icem.models import forward_model_from_string
from icem.misc.rollout_utils import RolloutManager
...
def get_controllers(params, env, forward_model):
    if params.initial_controller is None or params.initial_controller in [
        "none",
        "null",
        None,
    ]:
        ...
        if issubclass(controller_class, ModelBasedController):
            ...
            initial_controller = controller_class(
                env=env, **params.initial_controller_params
            )

    controller_class = controller_from_string(params.controller)
    if issubclass(controller_class, ParallelController):
        controller_params = recursive_objectify(
            params.controller_params, make_immutable=False
        )
    else:
        controller_params = params.controller_params
    if issubclass(controller_class, ModelBasedController):
        main_controller = controller_class(
    ...
    if main_controller.needs_data:
        if (
            ...
            or len(params.controller_data_sources) < 1
        ):
            raise AttributeError(
                "controller needs data to be trained but no source given"
            )
        for s in params.controller_data_sources:
            if s not in valid_data_sources:
                raise KeyError(
                    f"Invalid data source '{s}', valid ones are {''.join(valid_data_sources)}"
                )
...
class MainState:
    ...
    def load(self, file):
        ...
        if len(dat) == 1:
            ...
            allogger.get_logger("root").step_per_key = allogger.get_logger(
                "root"
            ).manager.dict(step_per_key)
        else:
            raise AttributeError(
...
def main():
    ...
    rollout_buffer_expert = (
        RolloutBuffer()
    )  # buffer for successful expert rollouts and dagger relabels
    ...
    if (
        "checkpoints" in params
    ):  # we could check whether we want to check for rollout_length consistency?
        checkpoint_manager = CheckpointManager(
            model_dir=params.model_dir, **params.checkpoints
        )
        ...
        if "evaluation_rollouts" in params and params.evaluation_rollouts > 0:
            checkpoint_manager.load_buffer(
                suffix="_eval", rollout_buffer=rollout_buffer_eval
            )
        ...
        checkpoint_manager = CheckpointManager(
            model_dir=params.model_dir, load=False, save=False
        )
    ...
    def save_checkpoint(cpm: CheckpointManager, final=False):
        ...
        if cpm is not None and cpm.save:
            if final or step % cpm.save_every_n_iter == 0:
                ...
                for _rollouts, _name in [
                    (rollout_buffer, ""),
                    (rollout_buffer_eval, "_eval"),
    ...
    postfix_dict = (
        {"Successful Rollouts": main_state.successful_rollouts}
        if main_state.successful_rollouts
        else {}
    )
    do_initial_rollouts = (
        initial_controller is not None and params.initial_number_of_rollouts > 0
    )
    ...
    if checkpoint_manager.do_restarting:
        if (
            main_state.iteration + checkpoint_manager.restart_every_n_iter
            < total_iterations
        ):
            current_max_iterations = (
                main_state.iteration
                + checkpoint_manager.restart_every_n_iter
                + 1 * do_initial_rollouts
            )
            print(
                f"Due to restarting we are only running {checkpoint_manager.restart_every_n_iter} iterations now"
            )
    ...
    for iteration in t_main:  # first iteration is for initial controller...
        ...
        if params.append_data:
            rollout_buffer.rollouts.extend(new_rollouts)
        else:
        ...
        # Evaluation runs
        if (
            not is_init_iteration
            and "evaluation_rollouts" in params
            and params.evaluation_rollouts > 0
        ):
            new_rollouts = RolloutBuffer(
                rollouts=rollout_man.sample(
                    controller,
                    render=params.rollout_params.render_eval,
                    mode="evaluate",
                    name="eval",
                    no_rollouts=params.evaluation_rollouts,
                    desc="eval_rollout",
                )
            )
            if "append_data_eval" in params and params.append_data_eval:
                rollout_buffer_eval.rollouts.extend(new_rollouts)
            else:
                rollout_buffer_eval = new_rollouts
            reward_info.update(
                compute_and_log_reward_info(new_rollouts, logger, prefix="eval_")
            )
        ...
        if "avg_return_required_to_solve" in params:
            ...
            if all(
                [
                    avg_return >= params.avg_return_required_to_solve
                    for avg_return in average_return_history
                ]
            ):
                min_time_required_to_solve = min(
                    min_time_required_to_solve, main_state.iteration
                )
            reward_info["required_iterations_to_solve"] = min_time_required_to_solve
            logger.info(
                f'Required iterations to solve: {reward_info["required_iterations_to_solve"]}'
            )

        reward_info_full.update(
            update_reward_dict(iteration, reward_info, reward_info_full)
        )
        save_checkpoint(checkpoint_manager)
